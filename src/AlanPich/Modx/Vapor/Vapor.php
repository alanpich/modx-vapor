 ...
 * Adapted from original vapor script, 2012 by MODX, LLC.
 *
 ...
 *
 * @package AlanPich\Modx\Vapor
 *
 */

namespace AlanPich\Modx\Vapor;
...
class Vapor
{
    ...
    public function vaporize($ModxBasePath, $config = array())
    {
        ...
        try {
                    ...
                    case 'modWorkspace':
                        ...
                        foreach ($modx->getIterator('modWorkspace', $classCriteria) as $object) {
                            ...
                            if ($package->put($object, $classAttributes)) {
                                ...
                                $modx->log(
                                    modX::LOG_LEVEL_WARN,
                                    "Could not package {$class} instance with pk: " . print_r($object->getPrimaryKey())
                                );
                            }
                        }
                        ...
                        if (isset($response['collection'])) {
                            foreach ($response['collection'] as $object) {
                                ...
                                if ($package->put($object, $classAttributes)) {
                                    ...
                                    $modx->log(
                                        modX::LOG_LEVEL_WARN,
                                        "Could not package {$class} instance with pk: " . print_r(
                                ...
                                if (!$sourceBases['pathIsRelative'] && strpos($source, '://') === false) {
                                    ...
                                    if (strpos($source, $base_path) === 0) {
                                        ...
                                        if (strpos($source, ':\\') !== false || strpos($source, ':/') !== false) {
                                            $sourceBasePath = str_replace(
                                                '\\',
                                                '/',
                                                substr($source, strpos($source, ':') + 1)
                                            );
                                        }
                                        $target = 'dirname(MODX_BASE_PATH) . "/sources/' . ltrim(
                                            dirname($sourceBasePath),
                                            '/'
                                        ) . '/"';
                                        $classAttributes['resolve'][] = array(
                                            'type' => 'file',
            ...
            $stmt = $modx->query(
                "SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = '{$modxDatabase}' AND TABLE_NAME NOT IN (" . implode(
                    ',',
                    $coreTables
                ) . ")"
            );
            ...
            if (is_array($extraTables) && !empty($extraTables)) {
                ...
                $excludeExtraTablePrefix = isset($vaporOptions['excludeExtraTablePrefix']) && is_array(
                    $vaporOptions['excludeExtraTablePrefix']
                ) ? $vaporOptions['excludeExtraTablePrefix'] : array();
                $excludeExtraTables = isset($vaporOptions['excludeExtraTables']) && is_array(
                    $vaporOptions['excludeExtraTables']
                ) ? $vaporOptions['excludeExtraTables'] : array();
                foreach ($extraTables as $extraTable) {
                    ...
                    if (isset($resultSet[1])) {
                        if ($addTablePrefix) {
                            $object['drop'] = "DROP TABLE IF EXISTS {$modx->escape(
                                '[[++table_prefix]]' . $extraTableName
                            )}";
                            $object['table'] = str_replace(
                                "CREATE TABLE {$modx->escape($extraTable)}",
                                "CREATE TABLE {$modx->escape('[[++table_prefix]]' . $extraTableName)}",
                                $resultSet[1]
                            );
                        } else {
                        ...
                        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            if ($addTablePrefix) {
                                $object['data'][] = "INSERT INTO {$modx->escape(
                                    '[[++table_prefix]]' . $extraTableName
                    ...
                    if (!$package->put($object, $attributes)) {
                        $modx->log(
                            modX::LOG_LEVEL_WARN,
                            "Could not package rows for table {$extraTable}: " . print_r($object, true)
                        );
                    } else {
            ...
            $modx->log(
                modX::LOG_LEVEL_INFO,
                sprintf("Vapor execution completed without exception in %2.4fs", $endTime - $startTime)
            );
        } catch (Exception $e) {
            if (empty($endTime)) {
                $endTime = microtime(true);
            ...
            if (!empty($modx)) {
                ...
                $modx->log(
                    modX::LOG_LEVEL_INFO,
                    sprintf("Vapor execution completed with exception in %2.4fs", $endTime - $startTime)
                );
            } else {
                echo $e->getMessage() . "\n";
            }
